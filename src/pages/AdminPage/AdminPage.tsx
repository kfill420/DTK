...
function AdminPage() {
  ...
  return (
    <div className='adminPage'>
      ...
      <form onSubmit={handleSubmit} className="adminPage_add">
        <div className="adminPage_add_group">
          ...
          <div className="adminPage_add_group_discount">
            ...
            <span className="adminPage_add_group_discount_symbol">{isPercentage(input.discount) ? "%" : "€"}</span>
          </div>
          <select name="products" id="product_id" className="adminPage_add_group_products" value={productSelected} onChange={handleChange}>
        ...
        <div className="adminPage_add_productsSelect">
          {
            productsSelected.length > 0 && productsSelected.map((product) => (
              <div key={product} className="adminPage_add_productsSelect_product">
        ...
        <button type="submit" className="adminPage_add_button" disabled={!(input.code.length > 4 && input.discount.length > 0 && productsSelected.length > 0)}>Ajouter</button>
      </form>
      <table>
        <thead>
          <tr>
